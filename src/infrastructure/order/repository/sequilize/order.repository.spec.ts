...
describe("Order repository test", () => {
    ...
    it("should update an order", async () => {
        ...
        // Update order: New customer and new product
        ...
        //update the order
        const orderUpdated = new Order(order.id, customer2.id, [orderItem2]);
        await orderRepository.update(orderUpdated);

        expect(orderUpdated.id).toBe(order.id);
        expect(orderUpdated.customerId).toBe(customer2.id);
        expect(orderUpdated.items).toHaveLength(1);
        expect(orderUpdated.items[0]).toStrictEqual(orderItem2);
    });
    ...
    it("should find an order by id", async () => {
        ...
        expect(orderFound.items).toStrictEqual(order.items);
    });
